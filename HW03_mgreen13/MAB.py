...
def greedy_update(time,reg_boolean,armToPull,armTrueExpectedReward):
    ...
    # return either regret or % of pulls on optimal arm
    if reg_boolean == False:
...
def ucb(time, reg_boolean,armToPull,armTrueExpectedReward):
    # by intitializing q at one, it forces the all arms to be played once 
    q = [0,0,0,0,0]
    reward_hist = {0:[],1:[],2:[],3:[],4:[]}
    rewards = []
    for i in range(5):
...
def plotting_function(list_regrets_random,list_regrets_greedy,list_regrets_e_first_greedy,list_regrets_e_greedy,title,ylabel):
    ...
    plt.plot(X,np.array(list_regrets_greedy)/1.5,color = "red",label='greedy')
...
plotting_function(list_regrets_random,list_regrets_greedy,list_regrets_e_first_greedy,list_regrets_e_greedy,"Gamble on 'Hard Bandit' Reward Distributions","Regret")
#
...
list_regrets_ucb,list_regrets_e_first_greedy = simulate_function(hardArmToPull,hardTrueExpectedReward,True,1000,100,True)
np.save("ucb_easy",np.array(list_regrets_ucb))
# HARD BANDIT, 100 gambles, ucb and epsilon with %

list_regrets_ucb,list_regrets_e_first_greedy = simulate_function(hardArmToPull,hardTrueExpectedReward,True,1000,100,True)
list_regrets_ucbP,list_regrets_e_first_greedyP = simulate_function(hardArmToPull,hardTrueExpectedReward,False,1000,100,True)
...
plt.legend(loc = 'upper left')
plt.title("Regret of with Top Two Strategies Played on Hard Bandit")
plt.xlabel("Gamble Duration")
...
plt.legend(loc = 'upper left')
plt.title("Average Percent of Optimal Pulls with Top Two Strategies Played on Hard Bandit")
plt.xlabel("Gamble Duration")
plt.ylabel("% Optimal Arm")
plt.show()


# ------------------------------------------------ EASY BANDIT SIMULATIONS AND PLOTS ---------------------------
centers = np.array([.4,.5,.6,.7,.8])
alphas,betas,hardTrueExpectedReward,hardArmToPull = dist_maker(200,centers,"Easy Bandit")

##HARD BANDIT, 100 GAMBLES, REGRET,
list_regrets_random,list_regrets_greedy,list_regrets_e_first_greedy,list_regrets_e_greedy = simulate_function(hardArmToPull,hardTrueExpectedReward,True,1000,1,False)
plotting_function(list_regrets_random,list_regrets_greedy,list_regrets_e_first_greedy,list_regrets_e_greedy,"Gamble on 'Easy Bandit' Reward Distributions","Regret")
...
plt.plot(X,list_regrets_ucb,color = "red",label = "UCB")
plt.legend(loc = 'upper left')
plt.title("Regret of with Top Two Strategies Played on Easy Bandit")
plt.xlabel("Gamble Duration")
plt.ylabel("Regret")
plt.show()


list_regrets_ucbP,list_regrets_e_first_greedyP = simulate_function(hardArmToPull,hardTrueExpectedReward,False,1000,20,True)
X = list(range(100,1000,10))
plt.plot(X,list_regrets_e_first_greedyP, color = 'green', label = 'e-first greedy')
plt.plot(X,list_regrets_ucbP,color = "red",label = "UCB")
plt.legend(loc = 'upper left')
plt.title("Percent of Optimal Pulls with Top Two Strategies Played on Easy Bandit")
plt.xlabel("Gamble Duration")
plt.ylabel("% Optimal Arm")
plt.show()
